                    ...
                    <div class="flex items-center gap-5 mb-6">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-1 mt-1">
                                <span v-for="i in 5" :key="i" class="text-yellow-400 text-sm" :class="i <= 4 ? '' : 'opacity-30'">★</span>
            ...
            <Card>
                ...
                <CardContent>
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-lg bg-muted flex items-center justify-center"><Truck class="w-6 h-6 text-muted-foreground" /></div>
                        <div>
                            <p class="font-semibold">Renault Master</p>
                            <p class="text-sm text-muted-foreground">AA-123-BB · Hub Paris Centre</p>
                        </div>
                        <Badge class="ml-auto bg-green-100 text-green-700 border-green-200 hover:bg-green-100">Actif</Badge>
                    </div>
                </CardContent>
            </Card>
...
function parseJwt(token: string): Record<string, unknown> | null {
    try { const b64 = token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/'); return JSON.parse(atob(b64)); } catch { return null; }
}
const token = useCookie('access_token');
const payload = computed(() => token.value ? parseJwt(token.value) : null);
const fullName = computed(() => { const first = payload.value?.firstname as string ?? ''; const last = payload.value?.lastname as string ?? ''; return `${first} ${last}`.trim() || 'Chauffeur'; });
const initials = computed(() => fullName.value.split(' ').map(n => n[0] ?? '').join('').toUpperCase().slice(0, 2));
</script>
